...
//              Answers.getInstance().logCustom(new CustomEvent("storagePermissionRationaleAccepted")); //1L
              // request permissions
...
//              Answers.getInstance().logCustom(new CustomEvent("storagePermissionRationaleDenied")); //1L
              // fall back if we can
  ...
  public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions,
    ...
    if (requestCode == REQUEST_WRITE_TO_SDCARD_PERMISSIONS) {
      if (grantResults.length == 1 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
         ...
         */
        havePermission = true;
//        Answers.getInstance().logCustom(new CustomEvent("storagePermissionGranted")); //1L
...
//        Answers.getInstance().logCustom(new CustomEvent("storagePermissionDenied")); //1L
        final File fallbackFile = getExternalFilesDir(null);
        if (fallbackFile != null) {
          quranSettings.setAppCustomLocation(fallbackFile.getAbsolutePath());
          checkPages();
...
//        Crashlytics.logException(ioe); //1L
      }
