...
//      Crashlytics.logException(e); //1L
  ...
  private DatabaseHandler(Context context,
    ...
    // if there's no Quran base directory, there are no databases //1L
    String base = quranFileUtils.getQuranDatabaseDirectory(context);
    if (base == null) return;

    String path = base + File.separator + databaseName;
//    Crashlytics.log("opening database file: " + path); //1L
...
//      Crashlytics.log("database file " + path + (new File(path).exists()? " exists" : " doesn't exist")); //1L
      throw se;
    }
